...
time_col <- dplyr::case_when(
	...
	"date" %in% names(panel_raw) ~ "date",
	TRUE ~ NA_character_
)

if(is.na(time_col)) {
	stop("Your input file has no 'date' or 'year column. Columns are: ", paste(names(panel_raw), collapse= ", "))
}
...
if("year" %in% names(panel_raw)) {
	...
	stop("Could not find a date or year column in social_infrastructure_wide_latest_with_years_2000_2025.csv")
}
...
#Ensure only data from 2001 - 2025 is shwon
panel_raw <- panel_raw %>%
	filter(year >= start_year & year <= end_year)
	
#Filter through countries with iso3c codes that exist and select the appropriate columns
...
if(any(dup_name_flags)) {
  message("Found duplicate indicator columns NAMES after cleaning.")
...
panel_complete <- grid %>%
  left_join(
    panel_latam50 %>% select(iso3c, year, all_of(ind_cols)),
    by = c("iso3c", "year")
  )	
#Drop indicators with >50% missingness

#Find the mean value of all missing data for each indicator column
...
#----Save Outputs-----
panel_csv <- file.path(out_dir, "sd_infrastructure_latam50_panel_25y.csv")
panel_xlsx <- file.path(out_dir, "sd_infrastructure_latam50_panel_25y.xlsx")

latest_csv <- file.path(out_dir, "sd_infrastructure_latest_country_wide.csv")
latest_xlsx <- file.path(out_dir, "sd_infrastructure_latest_country_wide.xlsx")
